import{f as c,c as d,d as o,h as n,u as r,i as h}from"./script-BmuZr8xh.js";class p{btn;petId;uid;constructor(){this.btn=document.querySelector(".info__buy");const t=new URLSearchParams(window.location.search);this.petId=t.get("id")||"",this.uid=this.getCookie("UID"),this.init()}init(){this.btn&&this.btn.addEventListener("click",()=>{this.uid?this.fetchUsers().then(t=>{t?this.addPatToUser(this.petId,t.id).then(()=>{window.location.href="/Dreamy-Tail-doplom/cabinet-user.html"}):window.location.href="/Dreamy-Tail-doplom/registration.html"}):window.location.href="/Dreamy-Tail-doplom/registration.html"})}getCookie(t){const s=`; ${document.cookie}`.split(`; ${t}=`);if(s.length===2)return s.pop()?.split(";").shift()}async fetchUsers(){const t=await c(d(o,"users")),e=[];return t.forEach(i=>{e.push({id:i.id,uid:i.data().uid,role:i.data().role})}),e.find(i=>i.uid===this.uid)}async addPatToUser(t,e){try{const s=n(o,"users",e);await r(s,{myPet:h(t)});const i=n(o,"pats",t);await r(i,{owner:e})}catch(s){console.error(s)}}}class u{cardHolder;patsArr;pet;petId;constructor(t){this.cardHolder=document.querySelector(t),this.patsArr=[],this.pet=[];const e=new URLSearchParams(window.location.search);this.petId=e.get("id")||"",this.init()}init(){this.conectDB()}async conectDB(){try{const t=d(o,"pats");(await c(t)).forEach(i=>{const a=i.data();this.patsArr.push({id:i.id||"",img:a.img||"",imgWebP:a.imgWebP,name:a.name||"No Name",fullInfo:a.fullInfo||"",view:a.view||"",breed:a.breed||"",age:a.age||""})});const s=this.patsArr.find(i=>i.id===this.petId);s&&this.pet.push(s),console.log(this.pet),this.renderPat(this.pet)}catch(t){console.error(t)}}renderPat(t){this.cardHolder.innerHTML="",t.forEach(e=>{const s=`
